...
export class Migration20220202103222 extends Migration {
  ...
  async up(): Promise<void> {
    ...
    this.addSql('alter table "decision" add column "user_id" uuid not null, add column "poll_id" uuid not null;');
    this.addSql('alter table "decision" drop constraint if exists "decision_answer_check";');
    this.addSql('alter table "decision" alter column "answer" type int4 using ("answer"::int4);');
    this.addSql('alter table "decision" alter column "answer" drop not null;');
