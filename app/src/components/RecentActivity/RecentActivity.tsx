    ...
    if (user && data && data.getActivity.length != 0) {
        return (
            <Wrapper>
                {data.getActivity
                    .slice(0, limit)
                    .map((activity: { name: string; date: number; type: string; id: string; pollId: string }, idx: number) => (
                        <LinkButton
                            key={idx}
                            primary={false}
                            active={true}
                            arrow={true}
                            link={`/result/${activity.id == activity.pollId ? activity.id : activity.pollId}`}
                        >
                            <ActivityContainer>
